...
//
// Table class for t94_logsub
//
class ct94_logsub extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't94_logsub';
		$this->TableName = 't94_logsub';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t94_logsub`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('t94_logsub', 't94_logsub', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// logmain_id
		$this->logmain_id = new cField('t94_logsub', 't94_logsub', 'x_logmain_id', 'logmain_id', '`logmain_id`', '`logmain_id`', 3, -1, FALSE, '`logmain_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->logmain_id->Sortable = TRUE; // Allow sort
		$this->logmain_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['logmain_id'] = &$this->logmain_id;

		// index_
		$this->index_ = new cField('t94_logsub', 't94_logsub', 'x_index_', 'index_', '`index_`', '`index_`', 16, -1, FALSE, '`index_`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// subj_
		$this->subj_ = new cField('t94_logsub', 't94_logsub', 'x_subj_', 'subj_', '`subj_`', '`subj_`', 200, -1, FALSE, '`subj_`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "t93_logmain") {
			if ($this->logmain_id->getSessionValue() <> "")
				$sMasterFilter .= "`id`=" . ew_QuotedValue($this->logmain_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "t93_logmain") {
			if ($this->logmain_id->getSessionValue() <> "")
				$sDetailFilter .= "`logmain_id`=" . ew_QuotedValue($this->logmain_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_t93_logmain() {
		return "`id`=@id@";
	}

	// Detail filter
	function SqlDetailFilter_t93_logmain() {
		return "`logmain_id`=@logmain_id@";
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t94_logsub`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t94_logsubview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t94_logsubview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t94_logsubadd.php?" . $this->UrlParm($parm);
		else
			$url = "t94_logsubadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t94_logsubedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t94_logsubadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t94_logsubdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "t93_logmain" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_id=" . urlencode($this->logmain_id->CurrentValue);
		}
		return $url;
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->logmain_id->EditValue = $this->logmain_id->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->logmain_id->Exportable) $Doc->ExportCaption($this->logmain_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->logmain_id->Exportable) $Doc->ExportField($this->logmain_id);
