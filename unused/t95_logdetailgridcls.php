...
$t95_logdetail_grid = NULL; // Initialize page object first

class ct95_logdetail_grid extends ct95_logdetail {
	...
	// Page object name
	var $PageObjName = 't95_logdetail_grid';

	// Grid form hidden field names
	var $FormName = 'ft95_logdetailgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t95_logdetail"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_logmaster_id") && $objForm->HasValue("o_logmaster_id") && $this->logmaster_id->CurrentValue <> $this->logmaster_id->OldValue)
			return FALSE;
		...
		if ($objForm->HasValue("x_subject") && $objForm->HasValue("o_subject") && $this->subject->CurrentValue <> $this->subject->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_date_") && $objForm->HasValue("o_date_") && $this->date_->CurrentValue <> $this->date_->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->logmaster_id->setOldValue($objForm->GetValue("o_logmaster_id"));
		...
		$this->date_->setOldValue($objForm->GetValue("o_date_"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t94_logmaster") {
			$this->logmaster_id->Visible = FALSE;
			if ($GLOBALS["t94_logmaster"]->EventCancelled) $this->EventCancelled = TRUE;
		}
